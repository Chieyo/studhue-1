...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    ...
    try {
      ...
      final userRecord = await _getUserByUsername(
        username,
      ); // Assuming this returns the user row
      ...
      final AuthResponse response = await SupabaseService.supabase.auth
          .signInWithPassword(email: email, password: password);

      if (response.user != null) {
        _logger.info(
          'Supabase login successful for user: ${response.user!.id}',
        );
        ...
        await prefs.setString(
          'username',
          username,
        ); // You can still store username for display
        ...
        _logger.warning(
          'Supabase login attempt returned no user, but no error.',
        );
        if (mounted) {
